function generateSquareWave(handles, fig1, bangDAQ, info)
%This function will generate the square wave voltage profile.

%Set up the data save path.
saveDataPath = info.saveDataPath;
absoluteSavePath = makeBhangmeterDirectory(saveDataPath, 'Square');
            
%Set up a plotting directory.        
savePlotPath = info.savePlotPath;
absolutePlotPath = makeBhangmeterDirectory(savePlotPath, 'Square');

%Lets make the parameters more understandable.
amplitude = handles.params(4);  %Units are in Volts.
frequency = handles.params(5);  %Units are in hz.
offset = handles.params(6); %Units are in volts.
...
squarePulse = amplitude*square(g) + offset;
outputData = squarePulse;
time = 1 : 2*frequency;
...
end  %End of the function generateSquareWave.m