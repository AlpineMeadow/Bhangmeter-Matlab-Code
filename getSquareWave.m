function squareWave = getSquareWave(fig, bangDAQ)
...
squareWaveGridLayout = uigridlayout(squareWave, [length(rowHeight) ...
    length(columnWidth)]);
squareWaveGridLayout.RowHeight = rowHeight;
squareWaveGridLayout.ColumnWidth = columnWidth;

squareWave.TitlePosition = 'centertop';
squareWave.Title = 'Square Wave';
squareWave.BorderWidth = 2;
squareWave.FontWeight = 'bold';
squareWave.FontSize = 15;

%Fill the UserData structure.
squareWave.UserData = struct('start', 1, ...
    'stop', 0, ...
    'amplitude', amp, ...
    'period', period, ...
    'phase', phase, ...
...
row = 1;
column = 1;
getParamLabel(squareWaveGridLayout, label, row, column);

%Get the start time parameter.
row = 1;
column = 3;
getParam(squareWaveGridLayout, startTime, row, column);
...
row = 2;
column = 3;
getParam(squareWaveGridLayout, amp, row, column);
...
row = 3;
column = 3;
getParam(squareWaveGridLayout, period, row, column);
...
row = 4;
column = 3;
getParam(squareWaveGridLayout, phase, row, column);

%Set up a second peak end time label.
label = 'Start';
row = 5;
column = [1 3];
getStartButton(squareWaveGridLayout, label, row, column, bangDAQ);

%Set up a stop button.
label = 'Stop';
row = 6;
column = [1 3];
getStopButton(squareWaveGridLayout, label, row, column);
